...
class VectorCompare {
public:
    double magnitude(const std::unordered_map<std::string, int>& concordance) {
        double total = 0;
        for (const auto& pair : concordance) {
            total += std::pow(pair.second, 2);
        }
        return std::sqrt(total);
    }

    double relation(const std::unordered_map<std::string, int>& concordance1,
                    const std::unordered_map<std::string, int>& concordance2) {
        double topvalue = 0;
        for (const auto& pair : concordance1) {
            auto it = concordance2.find(pair.first);
            if (it != concordance2.end()) {
                topvalue += pair.second * it->second;
            }
        }
        double mag1 = magnitude(concordance1);
        double mag2 = magnitude(concordance2);
        if (mag1 * mag2 != 0) {
    ...
    std::unordered_map<std::string, int> concordance(const std::string& document) {
        std::unordered_map<std::string, int> con;
...
int main() {
    ...
    std::unordered_map<int, std::unordered_map<std::string, int>> index;
    for (const auto& doc : documents) {
        index[doc.first] = v.concordance(doc.second);
    }
    ...
    for (const auto& idx : index) {
        double relation = v.relation(v.concordance(searchterm), idx.second);
        if (relation != 0) {
            matches.push_back({relation, documents[idx.first].substr(0, 100)});
        }
    }
